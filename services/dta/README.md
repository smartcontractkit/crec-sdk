...
This DTA (Digital Transfer Agent) service is a Go SDK for composing on-chain operations against DTA contracts and for decoding DTA CVN verifiable events into strongly-typed Go structures.
...
## What you get
- A single Decode function that base64-decodes and unmarshals CVN events into VerifiableEvent and a typed ConcreteEvent
...
## Decoding
CVN emits verifiable events as base64-encoded JSON. Use Decode to obtain a VerifiableEvent with a populated ConcreteEvent.
...
Decode usage:
```go
ve, err := dta.Decode(ctx, cvnEvent)
if err != nil { /* handle */ }
